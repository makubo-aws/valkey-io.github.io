...
authors= ["makubo-aws", "ikolomi"]

[extra]
featured = false
+++

With the winter months winding down, it's a fitting time to introduce the general availability of [Spring Data Valkey](https://github.com/valkey-io/spring-data-valkey), a new open source Spring Data module that provides first-class integration between [Valkey](https://valkey.io/) and the Spring ecosystem.
...
For the Java ecosystem, that means [Spring](https://spring.io/) and [Spring Data](https://spring.io/projects/spring-data).
...
Spring Data Valkey is officially supported by the Valkey project and can optionally be used with [Valkey GLIDE](https://glide.valkey.io/), one of Valkey's official client libraries, that brings new operational-excellence capabilities to Spring applications.
...
Spring Data Valkey was built to align closely with the established Spring Data programming model while supporting Valkey's Redis-compatible API, protocol, and data structures.
...
To move your existing Spring Data applications to use Spring Data Valkey, you update a few dependencies, switch to Valkey-specific package namespaces, and adjust configuration property names; after that, your application continues to operate using the same Spring Data APIs and behaviors it always has.
For a step-by-step guide, you can refer to the [Spring Data Valkey migration documentation](https://spring.valkey.io/commons/migration/).
...
In addition, it introduces first-class support for [Valkey GLIDE](https://glide.valkey.io/), one of Valkey's official client libraries.

- **[Replica selection and availability-zone–aware routing](https://valkey.io/blog/az-affinity-strategy/)** (when supported by your deployment) – Enables smarter read distribution and improved resilience by directing traffic to appropriate replicas across availability zones, while helping reduce inter-AZ network traffic and associated data transfer costs.
- **[Resilient Pub/Sub support](https://glide.valkey.io/concepts/client-features/pubsub-model/)** – Automatically restores Pub/Sub subscriptions after disconnections, failovers, or topology changes so applications continue receiving messages without custom resubscription logic.
- **[OpenTelemetry integration](https://glide.valkey.io/concepts/client-features/open-telemetry/)** – Provides built-in observability hooks that emit tracing and metrics data, enabling teams to monitor Valkey interactions using standard OpenTelemetry-compatible tooling.
...
In microservices architectures, caches are often deployed in clustered, multi-AZ environments.
...
With GLIDE, Spring Data Valkey [automatically discovers cluster topology](https://valkey.io/blog/az-affinity-strategy/), routes requests to the correct primary or replica node, and adapts to slot remapping or primary promotions without application restarts.
...
[GLIDE provides resilient Pub/Sub support](https://aws.amazon.com/blogs/database/how-habby-enhanced-resiliency-and-system-robustness-using-valkey-glide-and-amazon-elasticache/) by automatically restoring subscriptions after disconnections, failovers, or cluster changes.
...
To get started, explore the project on GitHub: [https://github.com/valkey-io/spring-data-valkey](https://github.com/valkey-io/spring-data-valkey).
For documentation and a step-by-step migration guide from Spring Data Redis, visit [https://spring.valkey.io/overview/](https://spring.valkey.io/overview/).
